...
| 变量名       | 类型   | 说明                       |
| --------- | ---- | ------------------------ |
| `UUID`    | 随意 | 用于连接验证的唯一 UUID（**必填**）   |
| `ID`      | 随意 | 自定义订阅路径标识，如 `123456` （**必填**）    |
| `IP`      | 随意 | 多个出口 IP，换行分隔             |
| `TXT`     | 随意 | 可选的订阅附加信息（如备注/公告）        |
| `PROXYIP` | 随意 | 备用反代地址，如 `1.2.3.4:443`   |
| `启用反代功能`  | 随意 | `true` 启用备用反代            |
| `NAT64`   | 随意 | `true` 启用 IPv4 → IPv6 转换 |
| `我的节点名字`  | 随意 | 节点名称（展示在订阅中，默认值：狂暴）      |
